...
var cube;
var model2clip;
var model2world, world2clip;
var cube2world, world2cube;
var view2worldL, world2viewL;
var world2view, proj2clip, view2proj;
var world2clipL;

var lightP, lightI, eyeP;
var flatOrRound = 0;

var DEPTH_RESOLUTION = 1024;
...
window.onload = function init()
{
    ...
    lightProgram = initShaders( gl, "light.vs", "light.fs" );
    // gl.useProgram( lightProgram );
    

    //create a texture to hold the depth component
    //the data type can be UNSIGNED_INT or UNSIGNED_SHORT
    depthTexture = Texture2D.create
    (gl, 
     Texture2D.Filtering.NEAREST, Texture2D.Wrap.CLAMP_TO_EDGE,
     DEPTH_RESOLUTION, DEPTH_RESOLUTION, 
     gl.DEPTH_COMPONENT, gl.DEPTH_COMPONENT, gl.UNSIGNED_INT, null, false);
    
    //create a frame buffer to render the light perspective to
    //You don't need to attach a color component to the frame 
    //buffer. We'll be disallowing writes to the color channels
    //anyway. This method saves us 1024^2 * 4 bytes of memory!
    depthFBO = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, depthFBO);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, 
			    gl.TEXTURE_2D, depthTexture.id, 0);
    ...
    // EXERCISE 4
    // Create a "cube", rotated it by 0.3 about the y-axis, scale it
    // by 0.3, and place it at (-0.1, 0.1, -1.5) in the world.
    //
    cube = new Sphere(gl, 4, 2);
    cube2world = Mat.translation(new PV(-0.1, 0.1, -1.5, false)).times(Mat.scale(0.3).times(Mat.rotation(1, 0.3)));
    ...
    translation = new PV(0, 0, -3.5, false);
    ...
    document.getElementById("lookAt").onclick = function () {
        lookAt();
        //lookAtL(lightP, rotated2world.times(new PV(true));
        lookAtL();
    };
    ...
    // EXERCISE 2
    function lookAtL () {
        // Calculate and set world2clipL.
        // light position is lightP
        // object center position is (0,0,0) in object coordinates....
        var center = rotated2world.times(new PV(true)); //object center
        var uz = (lightP.minus(center)).unit();
        var ux = new PV(0, 1, 0, false).cross(uz).unit();
        var uy = uz.cross(ux).unit();

        view2worldL = Mat.translation(lightP).times(new Mat(ux, uy, uz));

        var R = new Mat(ux, uy, uz).transpose();
        var T = Mat.translation(lightP.minus());

        world2viewL = R.times(T);
        proj2clipL = new Mat();

        world2clipL = proj2clipL.times(view2proj).times(world2viewL);
    ...
    proj2clip = Mat.scale(new PV(1 / aspect, 1, 1, true));
    ...
    function updateM2C () {
        ...
        // EXERCISE 1
        // Set model2world and world2clip
        ...
        // lightP is in world frame.
        // lightP = new PV(-10, 0, 10, true);
        // lightP = new PV(-2, 0, 2, true);

        lightP = new PV(-1, 1, 1, true);
...
function render() {
...

    {
        ...
        //setup the depth frame buffer, mask all color channels
        //to disable writes, for efficiency
        if (useFrameBuffer) {
            gl.viewport(0, 0, DEPTH_RESOLUTION, DEPTH_RESOLUTION);
	...
	gl.clear(gl.DEPTH_BUFFER_BIT);
        ...
        // EXERCISE 3
        // Set model2world and world2clip in the vertex shader for the
        // light's point of view.  You should see the sphere.  It will
        // be "fat" because proj2clipL is the identity.
        ...
        gl.uniformMatrix4fv(world2clipLoc, false, world2clipL.flatten());
        ...
        // EXERCISE 5
        // Now render the cube.  Which uniform variable has a different value?
        // You should see the cube blocking the lower right of the sphere.
        var model2worldLoc = gl.getUniformLocation( program, "model2world" );
        gl.uniformMatrix4fv(model2worldLoc, false, cube2world.flatten());

        var world2clipLoc = gl.getUniformLocation( program, "world2clip" );
        gl.uniformMatrix4fv(world2clipLoc, false, world2clipL.flatten());

        cube.render(gl, program, flatOrRound);
	...
	gl.uniform1i(gl.getUniformLocation(program, "depthTex"), 0);
	depthTexture.bind(gl);

        gl.uniform4fv(gl.getUniformLocation(program, "lightP"), 
                      lightP.flatten());
        gl.uniform4fv(gl.getUniformLocation(program, "lightI"),
                      lightI.flatten());
        gl.uniform4fv(gl.getUniformLocation(program, "eyeP"), 
                      eyeP.flatten());    
        
        // EXERCISE 6
        // Change useFrameBuffer to true at the top of render (above).
        // Pass in world2clipL
        var world2clipLLoc = gl.getUniformLocation( program, "world2clipL" );
        gl.uniformMatrix4fv(world2clipLLoc, false, world2clipL.flatten());


        
        // EXERCISE 7
        // Pass in world2clip from the eye's point of view.
        // Pass in model2world for the sphere.
        // You should see the sphere as usual.
        var world2clipLoc = gl.getUniformLocation( program, "world2clip" );
        ...
        // EXERCISE 8
        // Do the cube.  What uniform variable has to change?
        // The cube should block the sphere, but towards the upper left.
        var model2worldLoc = gl.getUniformLocation( program, "model2world" );
        gl.uniformMatrix4fv(model2worldLoc, false, cube2world.flatten());

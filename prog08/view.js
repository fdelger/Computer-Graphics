...
window.onload = function init()
{
    ...
    //picture = new Picture(gl, new PV(1, 1, -1, true), new PV(0, 0, 1, false), new PV(0, -1, 0, false), 2, 2);
    ...
    document.getElementById("ReflectRefract").onclick = function () {
        if(reflectRefract == 0)
            reflectRefract = 1;
        else
            reflectRefract = 0;
        console.log(reflectRefract);
...
function render() {
    ...
    gl.uniform1i(gl.getUniformLocation(program, "reflectOrRefract"), reflectRefract);

    var model2pictureLoc = gl.getUniformLocation( program, "model2picture" );
     gl.uniformMatrix4fv(model2pictureLoc, false,
                        picture.world2picture.times(model2world).flatten());

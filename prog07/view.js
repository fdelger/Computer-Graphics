...
window.onload = function init()
{
    ...
    function updateM2C () {
        ...
        // EXERCISE 2
        // The light position is (10, 0, 10) in thee world.
        // lightP is in model frame.

        //lightP = new PV(0,0,0,true);
        //lightP = Mat.translation(new PV(10,0,13,false)).times(rotated2world.times(object2rotated.times(model2object.times(lightP))));

        lightP = new PV(10,0, 10, true);
        lightP = object2model.times(rotated2object.times(world2rotated.times(lightP)));
        
        //now i have the light in the world
        console.log(lightP);

        // eyeP is the eye position in the model frame.
        eyeP = new PV(true);
        eyeP = object2model.times(rotated2object.times(world2rotated.times(view2world.times(eyeP))));
    ...
    flatOrRound = 0;
    var check = false;
     document.getElementById("flatOrRound").onclick = function () {
        
        if (check) {
            flatOrRound = 1;
            console.log(flatOrRound);
        }
        else {
            flatOrRound = 2;
            console.log(flatOrRound);
